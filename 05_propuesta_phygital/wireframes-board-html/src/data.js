...
const familyInfo = {
  "A · Configuración": "Configuración",
  "B · Vinculación y prueba física": "Vinculación y prueba física",
  "C · Armado": "Armado",
  "D · Espera y señal": "Espera y señal",
  "E · Cierre y recuperación": "Cierre y recuperación",
};
...
export const coverageStates = rows.map(([id, title, trigger, primary, continuity, objective, error, visual, action, secondary, family]) => ({
  id,
  coverageState: id,
  interaction: family,
  title,
...
export const coverageFamilies = Object.keys(familyInfo);

export function getCoverageState(id) {
  return coverageStates.find((state) => state.id === id) ?? coverageStates[0];
}

export function getDeliveryFrame(id) {
  return deliveryFrames.find((frame) => frame.id === id) ?? deliveryFrames[0];
}
