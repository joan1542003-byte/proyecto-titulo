# Marca «Vuelve a lo que querías hacer»: materiales

**Estado:** propuesta D-073, pendiente de decisión del autor. El fundamento está en el [documento 21](../21_marca-relevo-a-tiempo-2026-09-25.md).

| Archivo o carpeta | Qué es |
| --- | --- |
| `lamina-relevo-a-tiempo-2026-09-25.html` | Manual de marca. Carga Schibsted Grotesk desde Google Fonts, las fotos desde `imagenes/` y el aviso desde `sonido/`. |
| `logotipo/` | Logotipo «relevo» en tinta, papel y azul pasta; ícono «r» sobre tinta y sobre azul (512 px). SVG editables. |
| `kit-relevo-2026-09-25.html` | Kit de diseño: 97 iconos con sus pesos y retícula, componentes y dieciséis pantallas de la app, y diez plantillas de presentación. |
| `iconos/` | 97 iconos en SVG (retícula de 24, trazo de 1,75, extremos redondeados), con su nombre como título accesible. |
| `imagenes/` | Tres fotos de muestra con la receta de corrección del proyecto. |
| `sonido/firma-relevo.wav` | Copia de la firma sonora de D-071, generada con `../marca-suena-donde-empieza/sonido/firma-sonora.py`. |

El logotipo se dibujó con los contornos de Schibsted Grotesk (Henrik Kongsvoll, Bakken & Bæck, 2022), distribuida con la SIL Open Font License, que permite usarlos en logotipos. Los SVG no incluyen la fuente.

## Créditos de las fotos

Las fotos se obtuvieron mediante Openverse el 25 de septiembre de 2026 y tienen licencia CC0 1.0, que no exige atribución; se dan los créditos por transparencia. Solo demuestran el tratamiento. La identidad final debe usar una sesión propia en hogares reales, con consentimiento.

| Archivo | Título original | Autoría | Fuente | Licencia | Página |
| --- | --- | --- | --- | --- | --- |
| tratada-11 | Open book and a cup of tea | freestocks.org | Flickr | CC0 1.0 | https://www.flickr.com/photos/135396164@N05/29364151550 |
| tratada-83 | Still Items | Tim Arterbury | StockSnap | CC0 1.0 | https://stocksnap.io/photo/still-items-ATTVTHA1AW |
| tratada-22 | Writing Drawing | Green Chameleon | StockSnap | CC0 1.0 | https://stocksnap.io/photo/writing-drawing-8Y0EDX4VP9 |

Las actividades y horarios de la lámina son ejemplos, no datos de participantes.

---

## Registro de cambios (disclaimer)

### 2026-09-25 — 97 iconos

- **Qué cambió:** la carpeta `iconos/` pasó de 40 a 97 iconos, y el kit, de doce a dieciséis pantallas.
- **Cómo estaba antes:** 40 iconos; `hogar.svg` se retiró porque repetía a `inicio.svg`.
- **Por qué:** el autor pidió iconos completos para toda la app.

### 2026-09-25 — Kit de diseño e iconos v2

- **Qué cambió:** se añadió el kit de diseño, y la carpeta `iconos/` se reemplazó por el juego v2 de 40 iconos.
- **Cómo estaba antes:** `iconos/` tenía 18 iconos de la primera versión, con extremos rectos. Los archivos `salida.svg` y `cancelar.svg` pasaron a llamarse `parlante.svg` y `cerrar.svg`.
- **Por qué:** el autor pidió iconos de mejor nivel y piezas para la app y las presentaciones.

### 2026-09-25 — Iconos y sistema completo

- **Qué cambió:** se añadió la carpeta `iconos/`, y el manual incorpora formas, fondos, texturas, iconos, tono, retícula, movimiento y usos incorrectos.
- **Cómo estaba antes:** no había iconos propios y el manual no definía esos componentes.
- **Por qué:** el autor pidió desarrollar la marca en su totalidad.

### 2026-09-25 — Firma del autor

- **Qué cambió:** el título y el manual usan la firma «Vuelve a lo que querías hacer». La carpeta conserva su nombre para no romper enlaces.
- **Cómo estaba antes:** la firma era «Lo que querías hacer, a tiempo».
- **Por qué:** el autor cambió la firma.

### 2026-09-25 — Creación

- **Qué se añadió:** manual de marca, logotipo, íconos, fotos de muestra y copia del aviso sonoro.
- **Cómo estaba antes:** la propuesta vigente era D-072, con la firma «Antes de que sea después» y Radio Canada.
- **Por qué:** el autor pidió una marca clara para la comisión, hermosa y basada en el estudio del diseño reciente.
