...
Como respuesta se propuso Relevo: una aplicación Android y un objeto que se deja junto al primer paso de la actividad elegida —por ejemplo, el libro que se quiere leer— y que emitirá una señal breve de luz y sonido cuando se cumpla una condición configurada. Se desarrollaron un prototipo Android que prepara el recordatorio y emite un aviso sonoro, todavía sin una prueba completa en un teléfono real, y un protocolo para comparar la señal situada con una notificación digital. El aporte del objeto físico sigue siendo una hipótesis que esa comparación deberá confirmar o descartar.
...
In response, Relevo was proposed: an Android application and an object left next to the first step of the chosen activity (for example, the book one wants to read), which will emit a brief light and sound cue when a user-defined condition is met. An Android prototype was developed that sets up the reminder and plays a sound cue, not yet fully tested on a real phone, together with a protocol to compare the situated cue with a digital notification. The contribution of the physical object remains a hypothesis that this comparison must confirm or reject.
...
Con esos parámetros, la integración incorporará aplicación Android, condición observable, comunicación Bluetooth, vencimiento y estados de error, y se probarán permisos, pantalla bloqueada, cambio de aplicación, pérdida de conexión, batería baja, reinicio y órdenes tardías. Solo después se realizará un piloto doméstico de duración acotada, cuyo protocolo definirá participantes, días de uso, criterios de inclusión, consentimiento, registros y forma de retiro. Se observarán armado, reubicación, pérdida, carga, convivencia, abandono y posible habituación a la señal. Una prueba de dos días, como la prevista para el prototipo Android, puede mostrar dificultades de uso, pero no la formación de un hábito; para eso haría falta un seguimiento más largo y una medida de automaticidad adecuada a la actividad (Gardner et al., 2012).
